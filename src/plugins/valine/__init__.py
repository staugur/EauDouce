...
#: 插件描述信息,什么用处.
__description__ = "Valine,一款快速、简洁且高效的无后端评论系统"
#: Plugin Author
#: 插件作者
...
#: 返回插件主类
def getPluginClass():
    return ValineComment

#: 插件主类, 不强制要求名称与插件名一致, 保证getPluginClass准确返回此类
class ValineComment(PluginBase):
    """ 继承自PluginBase基类 """

    def register_tep(self):
        """注册模板入口, 返回扩展点名称及扩展的代码, 其中include点必须是实际的HTML文件, string点必须是HTML代码."""
        return {"blog_show_content": "4@valine/blog_content.html", "blog_show_script": "valine/blog_script.html"}
