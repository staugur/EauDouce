...
__plugin_name__ = "gitalk"
#: Plugin describes information. What does it do?
#: 插件描述信息,什么用处.
__description__ = "gitalk评论系统"
#: Plugin Author
#: 插件作者
...
if PLUGINS["gitalk"]["enable"] in ("true", "True", True):
    __state__   = "enabled"
else:
    __state__   = "disabled"
...
#: 返回插件主类
def getPluginClass():
    return GitalkComment

#: 插件主类, 不强制要求名称与插件名一致, 保证getPluginClass准确返回此类
class GitalkComment(PluginBase):
    """ 继承自PluginBase基类 """

    def register_tep(self):
        """注册模板入口, 返回扩展点名称及扩展的代码, 其中include点必须是实际的HTML文件, string点必须是HTML代码."""
        return {"blog_show_header": '<link rel="stylesheet" href="https://unpkg.com/gitalk/dist/gitalk.css">',"blog_show_content": "4@gitalk/blog_content.html", "blog_show_script": "gitalk/blog_script.html"}
