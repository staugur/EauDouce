...
def DownloadBoard(basedir, board_id, zipfilename, board_pins, total_number, ctime, etime, version, site, user_ip, user_agent, downloadUrl):
    """
    ...
    if len(board_pins) > 10000:
        board_pins = board_pins[:10000]
    #初始写入数据库
    try:
        if user_agent:
            # 解析User-Agent
